...
const Profile = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    const userToptenArtist = await getUserTopTenArtist();
    setTopTenArtist(userToptenArtist.data);

    const userToptenTracks = await getUserTopTenTrack();
...
const TopTenArtist = ({ topTenArtist }) => {
  ...
  return (
    <div className=' md:mr-11'>
      <div className=' flex justify-between space-x-10 items-center pb-10'>
        <div className='text-textwhite font-bold '>Top Artists of All Time</div>
        <div>
          <button className='border-2 border-textwhite truncate text-textwhite rounded-full px-6 py-2'>
            See More
          </button>
        </div>
      </div>
      {topTenArtist.map((items) => {
